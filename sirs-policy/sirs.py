    ...
    **Arguments:**
    ...
    def step(self, params, x):
        ...
        **Arguments:**
        ...
        """
        # TODO: what are alpha, beta, gamma?
        ...
        x_t[3] = torch.distributions.binomial.Binomial(
            R, probs=1.0 - torch.exp(-gamma)
        ).sample((1,))
        x_t[4] = torch.distributions.binomial.Binomial(
            S, probs=1.0 - torch.exp(-alpha * I / self._N)
        ).sample((1,))
        x_t[5] = torch.distributions.binomial.Binomial(
            I, probs=1.0 - torch.exp(-beta)
        ).sample((1,))
        x_t[0] = S + x_t[3] - x_t[4]
        x_t[1] = I + x_t[4] - x_t[5]
        x_t[2] = R + x_t[5] - x_t[3]
    ...
    def negative_log_likelihood(self, params, y):
        ...
        for t in range(len(y)):
            S, I, R = series[t]
            ll_rs = binomial(R, probs=1.0 - torch.exp(-gamma)).log_prob(y[t][0])
            ll_si = binomial(S, probs=1.0 - torch.exp(-alpha * I / self._N)).log_prob(
                y[t][1]
            )
            ll_ir = binomial(I, probs=1.0 - torch.exp(-beta)).log_prob(y[t][2])
